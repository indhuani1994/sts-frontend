  ...
  const toggleDrawer = (state) => () => {
    ...
    return (
      ...
      >
        ...
        {statsError && user.role === 'staff' && (
          <Typography sx={{ color: '#b91c1c', mb: 2 }}>{statsError}</Typography>
        )}
        {hrStatsError && user.role === 'hr' && (
          <Typography sx={{ color: '#b91c1c', mb: 2 }}>{hrStatsError}</Typography>
        )}
        ...
        <Box sx={{
          ...
          {(user.role === 'staff' ? summaryCards : hrSummaryCards).map((card, index) => (
            <Box
              key={index}
              sx={{
            ...
            >
              ...
              <Typography sx={{ fontSize: 22, fontWeight: 800, color: '#0f172a' }}>
                {(user.role === 'staff' ? statsLoading : hrStatsLoading) ? '...' : card.value}
              </Typography>
            </Box>
          ))}
        </Box>
        ...
        <Box sx={{
          ...
          <Typography sx={{ fontWeight: 700, color: '#0f172a', mb: 1 }}>
            {user.role === 'hr' ? "Today's Follow-ups" : "Today's Schedule (9:00 AM - 6:00 PM)"}
          </Typography>
          {(user.role === 'staff' ? statsLoading : hrStatsLoading) ? (
            <Typography sx={{ color: '#64748b' }}>Loading schedule...</Typography>
          ) : (user.role === 'staff' ? stats.todaySchedule.length === 0 : hrStats.todaySchedule.length === 0) ? (
            <Typography sx={{ color: '#64748b' }}>
              {user.role === 'hr' ? 'No follow-ups scheduled for today.' : 'No classes scheduled for today.'}
            </Typography>
          ) : (
            <Box sx={{ display: 'grid', gap: 1.5 }}>
              {(user.role === 'staff' ? stats.todaySchedule : hrStats.todaySchedule).map((item, idx) => (
                <Box
                  key={`${item._id || item.regId || idx}`}
                  sx={{
                    display: 'grid',
                ...
                >
                  ...
                  <Box>
                    <Typography sx={{ fontWeight: 700, color: '#0f172a', fontSize: 14 }}>
                      {user.role === 'hr' ? (item.enName || 'Enquiry') : (item.topic || 'Topic not set')}
                    </Typography>
                    <Typography sx={{ color: '#64748b', fontSize: 12 }}>
                      {user.role === 'hr' ? (item.enCourse || 'Course not set') : (item.course || 'Course not set')}
                    </Typography>
                  </Box>
                  <Box sx={{ textAlign: { xs: 'left', sm: 'right' } }}>
                    <Typography sx={{ color: '#1f2937', fontSize: 12, fontWeight: 600 }}>
                      {user.role === 'hr' ? (item.enStatus || 'Status') : (item.classType || 'Class')}
                    </Typography>
                    <Typography sx={{ color: '#94a3b8', fontSize: 11 }}>
                      {user.role === 'hr' ? (item.enMobile || '') : (item.staff || 'Staff')}
                    </Typography>
                  </Box>
                </Box>
              ))}
                ...
                >
                  <Box component="thead" sx={{ background: '#eff6ff' }}>
                    <Box component="tr">
                      ...
                      <Box component="th" sx={tableHeadCell}>Amount Paid</Box>
                  ...
                  <Box component="tbody">
                    {earnings.map((row) => (
                      <Box component="tr" key={row.enquiryId} sx={{ borderBottom: '1px solid rgba(148, 163, 184, 0.2)' }}>
                        <Box component="td" sx={tableCell}>{row.studentName}</Box>
                        <Box component="td" sx={tableCell}>{Number(row.totalPaid || 0).toFixed(2)}</Box>
                        ...
                        <Box component="td" sx={tableCell}>
                          <Box
                            sx={{
                              ...
                              fontSize: 11,
                              fontWeight: 700,
                              color: row.status === 'earned' ? '#166534' : row.status === 'refund' ? '#9f1239' : '#92400e',
                              background:
                                row.status === 'earned'
                                  ? 'rgba(34, 197, 94, 0.12)'
                                  : row.status === 'refund'
          ...
          <Box sx={{ p: 3 }}>
            ...
            <Box sx={{ display: "flex", alignItems: "center", mb: 3 }}>
              <Avatar
                src={
                  profile.staffImage
                    ? resolveFileUrl(profile.staffImage)
            ...
            <List dense>
              <ListItem>
                <ListItemText
                  primary="Address"
                  secondary={profile.staffAddress}
                />
              </ListItem>
              <ListItem>
                <ListItemText primary="Mobile" secondary={profile.staffMobile} />
              </ListItem>
              <ListItem>
                <ListItemText primary="Email" secondary={profile.staffMail} />
              </ListItem>
            </List>
            ...
            {/* Qualifications */}
            {profile.staffQualification?.length > 0 && (
              <>
                <Typography variant="subtitle1" sx={{ fontWeight: "bold", mb: 1 }}>
                  Qualifications
                </Typography>
                <List dense>
                  {profile.staffQualification.map((qual, i) => (
                    <ListItem key={i}>
            ...
            {/* Experience */}
            {profile.staffExperience?.length > 0 && (
              <>
                <Divider sx={{ my: 2 }} />
                <Typography variant="subtitle1" sx={{ fontWeight: "bold", mb: 1 }}>
                  Experience
                ...
                <List dense>
                  {profile.staffExperience.map((exp, i) => (
                    <ListItem key={i}>
